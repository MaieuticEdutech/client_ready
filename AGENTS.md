...
Laravel 13, Livewire 4, Tailwind 4, Pest 4, PostgreSQL. The Laravel app is at
the repository root.

Livewire components are **multi-file**: `resources/views/components/⚡<name>/`
holds `<name>.php` (the class) and `<name>.blade.php` (the markup). Create them
with `php artisan make:livewire <name> --mfc`.
...
link (`video_url`); an upload always wins, and a row with neither renders as
"footage coming soon" rather than as an empty box.

## Storage

Logos and films use separate disks — see the `filesystems.logos` and
`filesystems.films` config keys. Both default to the public disk; set
`FILM_DISK=s3` with Cloudflare R2 credentials in production.
...
Section and card entrances use the `.reveal` class, driven by an
IntersectionObserver in `resources/js/reveal.js`. Stagger an element inline
with `style="--reveal-delay: 160ms"`. Everything is disabled under
`prefers-reduced-motion`.

## Local setup

```sh
createdb client_ready && createdb client_ready_test
